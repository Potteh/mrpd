...
QBCore.Functions.CreateCallback('qb-inventory:server:attemptPurchase', function(source, cb, data)
    ...
    -- AGC Credit integration: let the player choose cash, debit (bank), or an
    -- eligible credit card. agc-credit performs the actual server-side charge.
    if GetResourceState('agc-credit') ~= 'started' then
        TriggerClientEvent('QBCore:Notify', source, 'Payment system is unavailable.', 'error')
        cb(false)
        return
    end

    local purchaseSource = source
    local purchaseShop = shop
    local purchaseSlot = itemInfo.slot
    local purchaseName = itemInfo.name
    local purchaseInfo = itemInfo.info
    local purchaseAmount = amount
    local purchasePrice = price

    exports['agc-credit']:RequestPayment(purchaseSource, purchasePrice, ('%s x%d'):format(purchaseName, purchaseAmount), function(success, reason, method)
        if not success then
            if reason and reason ~= 'Payment cancelled.' then
                TriggerClientEvent('QBCore:Notify', purchaseSource, reason, 'error')
            end
            cb(false)
            return
        end

        -- Re-check state after the asynchronous payment selection. This prevents
        -- granting stale/out-of-stock items if shop state changed while choosing.
        local currentPlayer = QBCore.Functions.GetPlayer(purchaseSource)
        local currentShop = RegisteredShops[purchaseShop]
        local currentItem = currentShop and currentShop.items[purchaseSlot]
        if not currentPlayer or not currentItem or currentItem.name ~= purchaseName or currentItem.amount < purchaseAmount then
            -- Payment succeeded but fulfillment can no longer safely complete.
            -- Log loudly rather than duplicating/refunding money through a second system.
            print(('[qb-inventory] AGC payment succeeded but fulfillment failed for player %s, item %s x%d, $%s. Manual refund may be required.'):format(purchaseSource, purchaseName, purchaseAmount, purchasePrice))
            TriggerClientEvent('QBCore:Notify', purchaseSource, 'Purchase could not be fulfilled after payment. Contact an administrator.', 'error')
            cb(false)
            return
        end

        if not CanAddItem(purchaseSource, purchaseName, purchaseAmount) then
            print(('[qb-inventory] AGC payment succeeded but inventory became full for player %s, item %s x%d, $%s. Manual refund may be required.'):format(purchaseSource, purchaseName, purchaseAmount, purchasePrice))
            TriggerClientEvent('QBCore:Notify', purchaseSource, 'Your inventory changed before checkout completed. Contact an administrator for a refund.', 'error')
            cb(false)
            return
        end

        AddItem(purchaseSource, purchaseName, purchaseAmount, nil, purchaseInfo, 'shop-purchase-' .. (method or 'agc'))
        currentShop.items[purchaseSlot].amount -= purchaseAmount
        TriggerEvent('qb-shops:server:UpdateShopItems', purchaseShop, currentItem, purchaseAmount)
